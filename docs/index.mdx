...
教程部分的示例代码采用的是[@sinoui/web-forms](form)作为 UI 展示的。如果使用的是 sinoui-components，可以参考[@sinoui/sinoui-components-forms](sinoui-components-forms)。教程代码虽然采用的是@sinoui/web-forms，但是可以直接使用@sinoui/sinoui-components-forms 中的组件替换@sinoui/web-forms 相对应的组件同样可运行示例代码。
...
我们可以给`Field`组件指定`relyFields`属性表示跟哪个表单域关联，同时指定`relyFn`函数属性定义依赖规则。
